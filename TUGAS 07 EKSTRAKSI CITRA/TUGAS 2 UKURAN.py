...
def process_image(file_path):
    ...
    for k, contour in enumerate(contours, start=1):
        ...
        plt.text(cX, cY - 7, f'Label = {k}', color='red', fontsize=10, fontweight='bold')
        plt.text(cX, cY, f'Area = {area:.2f}', color='blue', fontsize=10, fontweight='bold')
        plt.text(cX, cY + 7, f'Perim = {perimeter:.2f}', color='green', fontsize=10, fontweight='bold')
        plt.text(cX, cY + 14, f'X = {cX:.2f}', color='cyan', fontsize=10, fontweight='bold')
        plt.text(cX, cY + 21, f'Y = {cY:.2f}', color='cyan', fontsize=10, fontweight='bold')

    plt.show()

# Path gambar yang ingin diproses
file_path = 'C:/Users/Asus/Downloads/WILAYAH BLOK A.png'
